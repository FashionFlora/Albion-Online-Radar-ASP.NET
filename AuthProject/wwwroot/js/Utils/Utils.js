...
import { PlayersDrawing } from '/js/Drawings/PlayersDrawing.js';
import { HarvestablesDrawing } from '/js/Drawings/HarvestablesDrawing.js';
import { MobsDrawing } from '/js/Drawings/MobsDrawing.js';
import { ChestsDrawing } from '/js/Drawings/ChestsDrawing.js';
import { DungeonsDrawing } from '/js/Drawings/DungeonsDrawing.js';
var connection = new signalR.HubConnectionBuilder().withUrl("/packetHub").build();
...
connection.start()
    .then(function () {
        ...
        connection.invoke("GetItemsIds").catch(function (err) {
            console.error(err);
        });


        connection.invoke("GetMobsIds").catch(function (err) {
            console.error(err);
        });

        connection.invoke("StartPacketCapture").catch(function (err) {
            console.error(err);
        });
...
connection.on("ReceiveItemsIds", function (itemsIds) {


    playersDrawing.updateItemsInfo(itemsIds);


});

connection.on("ReceiveMobsIds", function (mobsIds) {


    mobsHandler.updateMobInfo(mobsIds);
...
connection.on("InvokeEvent", function (Parameters) {
...
connection.on("InvokeRequest", function (Parameters) {
